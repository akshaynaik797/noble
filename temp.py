...
def fetch(after, before):
    ...
    mails = service.users().messages()
    results = mails.list(userId='me', labelIds=['INBOX'],
                                              q=q)
    while results is not None:
        mails_doc = results.execute()
        results = mails.list_next(results, mails_doc)
        for i in mails_doc['messages']:
            mega_set.add(i['id'])
        print(len(mega_set))
        z = 1
        messages = mails_doc.get('messages', [])
            ...
            for message in messages[::-1]:
                try:
                    with mysql.connector.connect(**conn_data) as con:
                        cur = con.cursor()
                        q = "select * from noble_mails where `id`=%s limit 1"
                        data = (message['id'],)
                        cur.execute(q, data)
                        tag = cur.fetchone()
                        if tag is not None:
